...
class BaseTrainer:
    ...
    def train_network(self):
        ...
        try:
            ...
            for e in range(self.epochs):
                ...
                start = time.time()
                self.run_epoch(mode='train', e=e)

                if self.valid_loader is not None:
                    with torch.no_grad():
                        self.run_epoch(mode='valid', e=e)
    ...
    def run_epoch(self, mode: str, e: int):
        '''
        ...
        if self.metrics is not None:
            for m_idx, rm in enumerate(running_metrics):
                ...
                if self.tb_writer is not None:
                    try:
                        if m_idx == 0:
                            self.tb_writer.add_scalar(f'Avg Loss/{mode}', running_loss / n_batches, e)

                        self.tb_writer.add_scalar(f'Avg {self.metric_names[m_idx]}/{mode}', rm / n_batches, e)
